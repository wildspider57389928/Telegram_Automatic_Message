...
def generate_dialogue_from_news(news_summary: str) -> str:
    """تولید دیالوگ رسمی و خشک با حداقل 450 کلمه"""
    prompt = f"""
    خبر زیر را تحلیل کن و یک گفتگوی کاملاً رسمی، خشک و بی‌طرفانه بین دو تحلیلگر اقتصادی به نام 'علی' و 'سارا' بنویس.
    
    دستورالعمل عددی:
    - تعداد تبادل‌ها: دقیقا 24 نوبت (علی 12 بار، سارا 12 بار)
    - هر نوبت حداقل 15 کلمه و حداکثر 20 کلمه
    - (بیشتر از 500 نشود)کل دیالوگ بین 450 تا 500 کلمه
    - سبک: کاملاً خبری، بدون احساس، بدون شوخی، بدون تعجب
    - هر جمله حاوی داده، رقم، علت‑معلول یا پیش‌بینی
    - وجود نداشته باشد و فقط احساس ها و هیجانات کم در بین دو نفر فقط در حد تایید و رد کرد و کلماتی مانند "چه جالب" "بله همینطوره" و... کلمه عاطفی یا غیررسمی. 
    فرمت:
    علی: ...
    سارا: ...
    ...
    response = client.models.generate_content(
        ...
        config=types.GenerateContentConfig(
            system_instruction=(
                "تو یک تحلیلگر اقتصادی هستی. "
                "فقط اعداد، داده‌ها و روابط علت‑معلولی. "
                "وجود نداشته باشد و فقط احساس ها و هیجانات کم در بین دو نفر فقط در حد تایید و مکالمه معمولی (کلماتی مانند 'چه جالب' 'بله همینطوره' و ...) کلمه عاطفی یا غیررسمی. "
                "دقت کن دیالوگ دقیقاً 24 تبادل و حداقل 450 کلمه و حداکثر 500 کلمه داشته باشد."
            ),
            temperature=0.3
        )
    )
